...
async function createFakeGame(roundNumber, numberOfPlayers) {
    try {
        ...
        const responseNew = await axios.post(`${apiUrl}/game`, { round_number: roundNumber, pseudo: newPseudo });
        ...
        for (let i = 1; i < numberOfPlayers; i++) {
            ...
            const responseJoin = await axios.post(`${apiUrl}/game/${gameId}/join`, { pseudo });
...
async function createFakeRound(gameId) {
    try {
        const response = await axios.post(`${apiUrl}/round`, { current_round_number: 1, game_id: gameId });
        return { roundId: response.data.round_id };
    } catch (error) {
        console.error('Erreur lors de la création de la manche factice :', error.message);
        throw error;
    }
}
 ...
 * @param roundId Identifiant de la manche
...
async function updateFakeRound(roundId, winner) {
    try {
        ...
        console.error('Erreur lors de la mise à jour d\'une manche factice :', error.message);
        throw error;
    }
}
...
async function updateFakeGame(gameId, winner) {
    try {
        ...
        console.error('Erreur lors de la mise à jour d\'une partie factice :', error.message);
        throw error;
    }
}
...
async function generateFakeData() {
    ...
    try {
        ...
        const updateRound = updateFakeRound(round.roundId, game.playersId[Math.floor(Math.random() * 4)])

        // Mise à jour de la partie
        const updateGame = updateFakeGame(game.gameId, game.playersId[Math.floor(Math.random() * 4)])
    } catch (error) {
