      ...
      if (database === "MySQL" || database === "SQLite") {
        ...
        if (database === "MySQL") {
          toastRef.current.show({
            severity: "info",
            summary: "Succès",
            detail: "Toutes les données de la base MySQL ont été supprimés",
          });
        } else {
          toastRef.current.show({
            severity: "info",
    ...
    if (database === "") {
      ...
      if (database === "MySQL") {
        const response = await axios.get(
          "http://localhost:5000/api/mysql/export"
        );
        ...
        toastRef.current.show({
          ...
          summary: "Succès",
          detail:
            "Toutes les données de la base MySQL ont été exportées dans vos téléchargements",
        });
      } else if (database === "SQLite") {
        const response = await axios.get(
          "http://localhost:5000/api/sqlite/export"
        );
        const blob = new Blob([response.data], { type: "application/sql" });
        const blobURL = window.URL.createObjectURL(blob);
        const link = document.createElement("a");
        ...
        toastRef.current.show({
          severity: "info",
          summary: "Succès",
          detail: "Toutes les données de la base MySQL ont été exportées",
        });
      } else {
        const response = await axios.get(
          "http://localhost:5000/api/mongo/export",
          { responseType: "text" }
        );
        ...
        toastRef.current.show({
          severity: "info",
          summary: "Succès",
          detail: "Toutes les données de la base MongoDB ont été exportées",
        });
      }
    }
  };
      ...
      if (database === "MySQL" || database === "SQLite") {
        ...
        if (database === "MySQL") {
          toastRef.current.show({
            severity: "info",
            summary: "Succès",
            detail: "Des données aléatoires ont été générés dans la base MySQL",
          });
        } else {
          toastRef.current.show({
            severity: "info",
            summary: "Succès",
            detail:
              "Des données aléatoires ont été générés dans la base SQLite",
          });
        }
      } else {
        ...
        toastRef.current.show({
          ...
          summary: "Erreur",
          detail:
            "Des données aléatoires ne peuvent pas être générées pour MongoDB",
        });
      }
    }
  };
  ...
  return (
    <div className="data">
      ...
      <Button
        className="button"
        type="button"
        label="Importer des données factices"
        onClick={onImportFakeButtonClick}
      />
    </div>
  );
}
